...
-- local Crafting              = safeRequire(Features.Crafting.CraftingServiceUtils)
local Goop                  = safeRequire(Features.Currency.GoopRewardUtils)
-- local Recipes               = safeRequire(Features.Crafting.Recipes)
...
task.spawn(function()
    ...
    while task.wait(1.5) do
        ...
        --[[
        if Config.AutoUnlockMachine then
            pcall(function()
                local unlocks = nil
                local coins = 0
                ...
                pcall(function()
                    if DataService then
                        local data = type(DataService.Get) == "function" and DataService:Get() or DataService.data
                        if data then
                ...
                if not unlocks then
                    local u = getDataSource("unlocks")
                    unlocks = type(u) == "function" and u() or u
                    local c = getDataSource("coins")
                    coins = tonumber(type(c) == "function" and c() or c) or 0
                end
                
                if unlocks and not unlocks.craftingMachine then
                    if coins >= 1000000 then
                        Remotes.CraftingService.RemoteFunction:InvokeServer("requestUnlockMachine")
                    end
                end
            end)
        end
        ...
        if Config.AutoCraft then
            pcall(function()
                ...
                if not inventory or not unlocks then
                    local i = getDataSource("inventory")
                    inventory = type(i) == "function" and i() or i
                    local u = getDataSource("unlocks")
                    unlocks = type(u) == "function" and u() or u
                end
                
                if unlocks and unlocks.craftingMachine then
                    if Crafting and type(Crafting.getRecipes) == "function" then
                        for _, recipe in ipairs(Crafting.getRecipes()) do
                            if Crafting.canCraftRecipe(recipe, inventory, unlocks) then
                                Remotes.CraftingService.RemoteFunction:InvokeServer("requestCraft", recipe.id, 1)
                            end
                        end
                    end
                end
...
--[[
_UI.AddToggle(AutoCraftingTab, "AutoUnlockMachine", {
    Title = "Auto Unlock Machine",
    Description = "Automatically pays 1M coins to unlock the Crafting Machine",
    Default = Config.AutoUnlockMachine,
    Callback = function(val) Config.AutoUnlockMachine = val end
})

_UI.AddToggle(AutoCraftingTab, "AutoCraft", {
    Title = "Auto Crafting",
    Description = "Automatically crafts all available recipes when you have the ingredients",
    Default = Config.AutoCraft,
    Callback = function(val) Config.AutoCraft = val end
})

_UI.AddButton(AutoCraftingTab, {
    Title = "Auto Collect Recipes",
    Description = "Teleports through all zones to fully load and collect every hidden recipe",
    Callback = function()
        task.spawn(function()
            ...
            if not root or not zonesFolder then return end
            
            local rs = game:GetService("ReplicatedStorage")
            local zsc
            pcall(function() zsc = require(rs.Source.Features.Zones.ZonesServiceClient) end)
            
            local allRecipeIds = {"crafty", "thorn", "geode", "slimeSlimeSlime", "puffy", "astro", "sunny", "melly"}
            local originalCFrame = root.CFrame
            
            for zoneId = 1, 15 do
                local zoneFolder = zonesFolder:FindFirstChild(tostring(zoneId))
                if zoneFolder then
                    if zsc and type(zsc.teleportToZone) == "function" then
                        pcall(function() zsc:teleportToZone(zoneId) end)
                    else
                        local part = zoneFolder:FindFirstChildWhichIsA("BasePart", true)
                        if part then root.CFrame = part.CFrame end
                    end
                    ...
                    if recipeObj then
                        local tCFrame = nil
                        pcall(function() tCFrame = recipeObj:GetPivot() end)
                        if tCFrame then
                            root.CFrame = tCFrame
                            task.wait(0.2)
                            for _, rId in ipairs(allRecipeIds) do
                                pcall(function()
                                    Remotes.CraftingService.RemoteFunction:InvokeServer("requestClaimRecipe", rId, recipeObj)
                                end)
                                task.wait(0.05)
                            end
                        end
            ...
            game:GetService("StarterGui"):SetCore("SendNotification", {
                ...
                Text = "Checked all zones and collected available recipes!",
                Duration = 5
            })
        end)
    end
